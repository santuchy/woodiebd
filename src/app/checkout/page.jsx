...
const AREAS = [
  {
    value: "inside_dhaka",
    label: "Inside Dhaka City (ঢাকা সিটির ভিতরে)",
    fee: 100,
  },
  {
    value: "outside_dhaka",
    label: "Outside Dhaka City (সাভার, আশুলিয়া, কেরানীগঞ্জ)",
    fee: 130,
  },
  {
    value: "others",
    label: "Others (অন্যান্য জেলা)",
    fee: 150,
  },
];
...
export default function CheckoutPage() {
  ...
  const qtyFromUrl = Number(sp.get("qty") || 1);

  const product = useMemo(() => {
    return PRODUCTS.find((p) => p.id === productId);
  }, [productId]);

  const [qty, setQty] = useState(Number.isNaN(qtyFromUrl) ? 1 : qtyFromUrl);

  const [area, setArea] = useState(AREAS[0].value);

  const selectedArea = useMemo(
    () => AREAS.find((a) => a.value === area),
    [area]
  );
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="rounded-lg border border-gray-300 bg-white p-6 text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">
              URL এ product id ঠিক আছে তো? <br />
              Example: <span className="font-semibold">/checkout?product=key-holder</span>
            </p>
          </div>
        </div>
      </section>
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-sm font-bold text-[#785E4C]">
                          ৳{product.price}
                        </p>
                      </div>
